...
for i in range(1, len(MACD)):
    if MACD[i] > SIGNAL[i] and MACD[i - 1] <= SIGNAL[i - 1]:
        # Buy shares
        price = y[i]  # Assuming buying at the closing price of the current day
        ...
        # Sell shares
        price = y[i]  # Assuming selling at the closing price of the current day
        shares_sold = shares_owned   # Selling all owned shares
        ...
        transaction_stats['Profit/Loss'].append((price - y[i - 1]) * shares_sold)  # Profit/Loss calculation
